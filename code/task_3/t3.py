...
i=4                                                                   #Done for image 0 and 4 in report
...
good_p = []
for p in range(203):
    const = np.dot(list_kp1_hom[p],Ess)
    good_points.append(float(np.dot(const,np.transpose(list_kp1_hom[p]))))
print(good_points)
index=[]
for el in range(203):
...
cv.imwrite('../../output/task_3/Feature Match with good points 1 - ' + str(i)+'.png', img2_n1)
cv.imshow('Feature Matching with good points 2',img2_n2)#,plt.show()
cv.imwrite('../../output/task_3/Feature Match with good points 2 - ' + str(i)+'.png', img2_n2)
...
fig.savefig('../../output/task_3/Plot '+ str(i) + '.png')
plt.show()